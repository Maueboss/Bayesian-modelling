...
## Master variables
RUN_DATE <- ymd("2024-07-09")
#RUN_DATE <- ymd("2008-08-11")

election_day <- ymd("2024-11-05")
start_date <- as.Date("2023-10-01") # Keeping all polls after March 1, 2008
...
model <- rstan::stan_model("sources/model/poll_model_2020.stan")
...
#model <- cmdstanr::cmdstan_model("us-potus-model/scripts/model/poll_model_2020.stan",compile=TRUE,force=TRUE)
#fit <- model$sample(
#  data = data,
#  seed = 1843,
...
jags_model <- jags.model("sources/model/poll_model_2020.jags", data = data, n.chains = 3)

# Burn-in
update(jags_model, 1000)

# Sampling
samples <- coda.samples(jags_model, variable.names = c("mu_b", "mu_c", "mu_m", "mu_pop", "e_bias", "polling_bias"), n.iter = 5000)
